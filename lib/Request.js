...
module.exports = Request = (function() {
  ...
  /*
   Ask a question and hide the answer with *
  */


  Request.prototype.hidden = function(query, callback) {
    _this = this;
    var stdin = process.openStdin();
    var listenhere = function(char) {
      char = char + "";
      switch (char) {
        case "\n":
        case "\r":
        ...
        default:
          process.stdout.write("\033[2K\033[200D" + query + 
             Array(_this.shell["interface"]().line.length+1).join("*"));
          break;
      }
    };
    process.stdin.on("data", listenhere);
 
    _this.shell["interface"]().question(query, function(value) {
      _this.shell["interface"]().history = _this.shell["interface"]()
                                                .history.slice(1);
      process.stdin.removeListener('data', listenhere);
      callback(value);
    });
  }
