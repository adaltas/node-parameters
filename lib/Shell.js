...
var Shell = module.exports = function(options){
  ...
  this.options.stdin = this.options.stdin || process.stdin;
  ...
  var dirs = mod._nodeModulePaths(__dirname);
  for(var i=0; i<dirs.length; i++){
    if(path.existsSync(dirs[i]+'/node_modules') || path.existsSync(dirs[i]+'/package.json')){
      this.project_dir = dirs[i];
      break;
    }
  }
  // Start
...
Shell.prototype.run = function(command){
  ...
  function next(err){
    ...
    if(!layer){
      return res.red('Command failed to execute'), res.prompt();
    }
...
Shell.prototype.prompt = function(){
  if(this.isShell){
    this.interface.question(this.styles.raw(this.options.prompt,{color: 'green'}), this.run.bind(this));
  }else{
    this.styles.ln();
    process.exit();
  }
